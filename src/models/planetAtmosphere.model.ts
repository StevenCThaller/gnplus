...
export default class PlanetAtmosphere extends BaseEntity {
  ...
  @ManyToOne(() => AtmosphereType, (atmosphereType) => atmosphereType.planetAtmospheres, {
    cascade: ["insert"],
  ...
  @OneToOne(() => PlanetaryBody, (planetaryBody) => planetaryBody.planetAtmosphere, {
    createForeignKeyConstraints: false
  })
  ...
  public static convertScan(data: PlanetScanData): PlanetAtmosphereParams | undefined {
    if (!data.Atmosphere && !data.AtmosphereType && !data.AtmosphereComposition) return;
    ...
    return {
      ...
      atmosphereComposition: data.AtmosphereComposition?.map(
        (comp: AtmosphereCompositionData): AtmosphereCompositionParams => ({
          atmosphereType: comp.Name,
